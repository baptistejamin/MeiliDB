...
impl PositiveBlob {
    ...
    pub fn write_to_bytes(&self, bytes: &mut Vec<u8>) {
        let map_vec = self.map.as_fst().to_vec(); // FIXME patch to have a as_slice() function
        bytes.write_u64::<LittleEndian>(map_vec.len() as u64).unwrap();
        bytes.extend_from_slice(&map_vec);
...
mod tests {
    ...
    fn serialize_deserialize() -> Result<(), Box<Error>> {
        ...
        positive_blob.write_to_bytes(&mut bytes);

        let bytes_len = bytes.len();
        let bytes = Arc::new(bytes);
        let positive_blob: PositiveBlob = PositiveBlob::from_shared_bytes(bytes, 0, bytes_len)?;
